            ...
            for (int loop = 0; loop < waveUpdateLoop; loop++)
            {
                ...
                for (int i = 0; i < hitCount; i++)
                {
                    ...
                    for (int b = bufferCenter - bufferWidth; b <= bufferCenter + bufferWidth; b++)
                    {
                        ...
                        waveBuffer[currentBuffer][b] = otherRigidbody.linearVelocity.y * interactMultiplier / width;
                        if (b < bufferCenter)
                        {
                            waveBuffer[currentBuffer][b] += interactHorizontalMultiplier * (-otherRigidbody.linearVelocity.x) * 2.0f;
                        }
                        else if (b > bufferCenter)
                        {
                            waveBuffer[currentBuffer][b] += interactHorizontalMultiplier * (otherRigidbody.linearVelocity.x);
                        }
